...
class Requester:
    ...
    def post(self, url="", payload={}):
        result = requests.post(self.get_url(url), json=payload)
        text = result.text

        return self.decode(result.status_code, text)

    def put(self, url="", payload={}):
        result = requests.put(self.get_url(url), json=payload)
        text = result.text

        return self.decode(result.status_code, text)

    def patch(self, url="", payload={}):
        result = requests.patch(self.get_url(url), json=payload)
        text = result.text

        return self.decode(result.status_code, text)

    def delete(self, url=""):
        result = requests.delete(self.get_url(url))
        text = result.text
